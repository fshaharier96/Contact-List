...
/**
 * Contacts import/exports
 */
class ImportExport {
	/**
	 * Import users from excel/csvs
	 *
	 * @return void
	 */
	...
	public function export() {
		...
		if ( $result->num_rows > 0 ) {
			...
			fputcsv( $output, [ 'First Name', 'Last Name', 'Email', 'Phone', 'Country' ] );

			// Loop through database results and write each row to the CSV file
			while ( $row = $result->fetch_assoc() ) {
				fputcsv( $output, [ $row['first_name'], $row['last_name'], $row['email'], $row['phone'], $row['country'] ] ); // Adjust these fields according to your database schema
			}
